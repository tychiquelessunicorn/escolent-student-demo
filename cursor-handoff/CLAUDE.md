...
- **A day-streak badge reached Today and shipped**, in direct violation of the no-gamification principle above: an "N-day streak" pill in the header, backed by `streak`/`demoStreak` counters in local storage that were bumped to 4 on completing the demo skill. It was ported straight out of the archived prototype, which still contains it — `claude-design-source/Today.dc.html` renders a flame emoji plus `{{ streak }}-day streak`, and `demo-investor.js` carries `getStreak`/`setStreak`/`incrementStreak`. **The prototype is the reference implementation for everything except this.** Port its behavior as instructed, but do not port the streak, and do not treat its presence there as evidence the mechanic was approved. Removed in full: badge, storage keys, and the reader.
